...
volatile uint8_t number_to_delay;
const uint8_t delay = 1000; //Dla przerwania czasowego 1kHz -> 1000 = 1s
...
// jeśli nie zadziałą usunąć
// Funkcja odpowiedzialna za obsługę przerwań sprzętowych
void HAL_SYSTICK_Callback(void){
	number_to_delay ++;
	if(number_to_delay == delay){
		count_number();
		number_to_delay = 0;
	}
...
int main(void)
{
  ...
  while (1)
  {
	  // jeśli nie zadziała przerwanie użyć to
//	  	HAL_Delay(500);
//	  	count_number();
