...
class BiometricGraph:
    ...
    # def delete_vertex(self, vertex):
    #     idx_vertex = self.indexes.pop(vertex)
    #     del self.objects_to_indexes[vertex]
    ...
    def rotation(self, translation, angle):
        ...
        for vertex in self.indexes:
            ...
            self.set_key_graph(vertex, (y_prim, x_prim))
    ...
    def set_key_graph(self, node, new_key):
        node.set_key(new_key)
        self.objects_to_indexes = {key : value for value, key in enumerate(self.indexes)}
...
def merge_near_vertices(graph: BiometricGraph, thr):
    ...
    #     centre_vertex = Node((y,x)) # pomyśleć wcześniej było w pętli (zakomentowane)
    #     for el_n in neighbours_list:
    #         y1, x1 = el_n[0].get_key()
    #         l = int(((y1 - y)2 + (x1-x)2)(1/2))
    #         iloczyn_skalarny = xx1 + yy1
    #         l1 = np.sqrt(x2 + y2)
    #         l2 = np.sqrt(x12 +  y1*2)
    ...
    for row in to_add:
        ...
        for vertex in row:
            if vertex not in deleted:
                ...
                for neighbour in neighbours:
                    if neighbour not in to_connect and neighbour not in row:
                        to_connect.append(neighbour)
        ...
        for vertex in row:
            if vertex not in deleted:
                ...
                print('przed',vertex,deleted)
        mean_vertex = Vertex((y_med, x_med))
        for vertex in to_connect:
            y_cur, x_cur = vertex.get_key()
            len_of_mean = np.sqrt(y_med**2 + x_med**2)
            len_of_current = np.sqrt(y_cur**2 + x_cur**2)
            iloczyn_skalarny = x_cur*x_med + y_cur*y_med
            angle = np.arccos(iloczyn_skalarny/(len_of_mean * len_of_current))
            distance = np.sqrt((y_med - y_cur)**2 + (x_med - x_cur)**2)
...
# img = cv2.imread('C:/studia/asd/lab11/Retina_graph_easy_3.png')
